-- Remove duplicate employees, keeping only the first one for each employee_id
...
-- Now add unique constraint to employee_id
ALTER TABLE public.employees ADD CONSTRAINT employees_employee_id_unique UNIQUE (employee_id);

-- Also add unique constraint to product_code  
ALTER TABLE public.products ADD CONSTRAINT products_product_code_unique UNIQUE (product_code);

-- Now create the assignments table with proper foreign keys
